...
const CATEGORIAS_POR_TIPO_PARCEIRO: Record<
  string,
  number[]
> = {
  SOLIDARIO: [8],
...
const CATEGORIA_ICON_MAP: Record<
  number,
  React.ReactNode
> = {
  1: (
    <Building2 className="w-6 h-6 text-green-primary" />
  ),
  ...
  3: (
    <GraduationCap className="w-6 h-6 text-green-primary" />
  ),

  4: (
    <Hotel className="w-6 h-6 text-green-primary" />
  ),

  5: (
    <Utensils className="w-6 h-6 text-green-primary" />
  ),
  ...
  7: (
    <Store className="w-6 h-6 text-green-primary" />
  ),

  8: (
    <HeartHandshake className="w-6 h-6 text-green-primary" />
  ),
};
...
const TIPO_PARCEIRO_ICON_MAP: Record<
  string,
  React.ReactNode
> = {
  SOLIDARIO: (
    <HeartHandshake className="w-5 h-5 text-green-primary" />
  ),
  ...
  INSTITUCIONAL: (
    <Landmark className="w-5 h-5 text-green-primary" />
  ),
};
...
export function CriarParceiroModal({
  ...
  const [
    novoParceiro,
    setNovoParceiro,
  ] = useState<NovoParceiroPayload>(
    NOVO_PARCEIRO_INICIAL
  );
  ...
  const buscarCoordenadasPorTexto =
    async () => {
  ...
  >(
    ...
    // --------------------------------------------------------
    // invalida verificação de disponibilidade
    // --------------------------------------------------------
  ...
  const getCategoriasPermitidas =
    () => {
      ...
      return CATEGORIA_OPTIONS.filter(
        ({ value }) =>
  ...
  const verificarEmailEDocumento =
    async (): Promise<boolean> => {

      const email =
        novoParceiro.email.trim();
      ...
      if (
        ...
        setErroFormulario(
          "Informe o e-mail e o documento."
        );
  ...
  const validarPasso2 =
    (): boolean => {
      ...
      if (
        !Number.isFinite(
          novoParceiro.latitude
        ) ||
        !Number.isFinite(
          novoParceiro.longitude
        ) ||
        (
          novoParceiro.latitude ===
            0 &&
  ...
  const proximoPasso =
    async () => {
      ...
      const disponivel =
        await verificarEmailEDocumento();
  ...
  const handleSubmit =
    async (
      ...
      try {
        ...
        const temIndicador =
          Boolean(
            novoParceiro.parceiroIndicadorId
          );
        ...
        const payloadFinal:
          ...
          aceiteMarketing:
            Boolean(
              novoParceiro.aceiteMarketing
            ),


          responsavelLegal:
            novoParceiro.responsavelLegal
              ? novoParceiro.responsavelLegal.trim()
              : null,


          responsavelLegalCpf:
            novoParceiro.responsavelLegalCpf
              ? somenteNumeros(
                  novoParceiro.responsavelLegalCpf
                )
              : null,
          ...
          cidade:
            novoParceiro.cidade.trim(),


          bairro:
            novoParceiro.bairro.trim(),
          ...
          expectativaGeracao:
            Number(
              novoParceiro.expectativaGeracao
            ) || 0,


          capacidadeBombona:
            Number(
              novoParceiro.capacidadeBombona
            ) || 0,
          ...
          <div className="p-4 bg-white-50/60 rounded-xl border border-white-100 space-y-3">
            ...
            <div className="pt-1">
              ...
              <label className="block">
                ...
                <select
                  value={
                    novoParceiro.parceiroIndicadorId?.toString() ||
                    ""
                  }
                  onChange={(e) => {
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                ...
                {novoParceiro.redesSociais.map(
                  (
                    ...
                    <span
                      key={`${rede}-${i}`}
                      className="flex items-center gap-1 rounded-md bg-white border border-white-200 px-2 py-0.5 text-xs text-black-primary"
                    >
                      ...
                      <button
                        ...
                        className="text-white-400 hover:text-red-primary cursor-pointer"
                      >

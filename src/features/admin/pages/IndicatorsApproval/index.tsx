...
function obterNomeOuRazaoSocial(
...
function IndicadorModal({
  ...
  const [form, setForm] =
    useState<FormIndicador>(FORM_INICIAL);
  ...
  useEffect(() => {
    ...
    if (indicador) {
      setForm({
        ...
        telefone: formatarTelefoneInput(
          indicador.telefone ?? ""
        ),
        site: indicador.site ?? "",
...
export function ParceirosIndicadores() {
  ...
  const [
    ...
    useState<ParceiroIndicador | null>(
      null
    );

  const [
    indicadorExclusao,
    setIndicadorExclusao,
  ] =
    useState<ParceiroIndicador | null>(
      null
    );
  ...
  const [
    modoEdicaoDetalhes,
    setModoEdicaoDetalhes,
  ] = useState(false);

  const [
    indicadorEditandoDetalhes,
  ...
  const carregarIndicadores =
    useCallback(async () => {
      ...
      try {
        const dados =
          await adminIndicadorService.listar();
...
const indicadoresFiltrados = useMemo(() => {
  ...
  const filtrados = indicadores.filter((indicador) => {
    ...
    const tipoCorresponde =
      !tipoFiltro || indicador.tipo === tipoFiltro;
  ...
  // 1º ATIVOS
  // 2º INATIVOS
  // Dentro de cada grupo: ID crescente
  // ==========================================
  ...
  const indicadoresPaginados =
    useMemo(() => {
      ...
      return indicadoresFiltrados.slice(
        inicio,
        fim
      );
    }, [
  ...
  // Se excluir/filtrar e a página atual deixar de existir,
  ...
  const salvarIndicador = async (
    ...
    try {
      if (indicadorEditando) {
        await adminIndicadorService.atualizar(
          indicadorEditando.id,
          dados
        );

        addToast(
          "Parceiro indicador atualizado com sucesso!",
          "success"
        );
      } else {
        await adminIndicadorService.criar(
          dados
        );

        addToast(
          "Parceiro indicador criado com sucesso!",
          "success"
        );
      }
      ...
      console.error(
        "Erro ao salvar parceiro indicador:",
        error
      );
  ...
  // ============================================================
  // SALVAR EDIÇÃO DETALHES
  // ============================================================

  const handleSalvarEdicaoDetalhes =
    async () => {
      if (
        !indicadorEditandoDetalhes
      ) {
      ...
      if (
        !indicadorEditandoDetalhes.nome?.trim()
      ) {
        addToast(
      ...
      try {
        await adminIndicadorService.atualizar(
          indicadorEditandoDetalhes.id,
          {
            nome:
              indicadorEditandoDetalhes.nome,

            tipo:
              indicadorEditandoDetalhes.tipo,
            ...
            email:
              indicadorEditandoDetalhes.email ||
              undefined,
            ...
            site:
              indicadorEditandoDetalhes.site ||
              undefined,
  ...
  const confirmarExclusao =
    async () => {
      ...
      try {
        await adminIndicadorService.excluir(
          indicadorExclusao.id
        );

        addToast(
          "Parceiro indicador excluído com sucesso!",
          "success"
        );
        ...
        console.error(
          "Erro ao excluir parceiro indicador:",
          error
        );
            ...
            <tbody>
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={7}
                    className="p-8 text-center text-white-500"
                  >
